...
func (r *TasksRepo) Add(t Task) (int64, error) {
	...
	var id int64
	err := r.QueryRow(q, t.Name, t.Description, t.StatusID, t.ProjectID, t.PriorityID).Scan(&id)
	return id, err
}
...
func (r *TasksRepo) Get(id int64, pid int64) (Task, error) {
	...
	var task Task
	err := r.QueryRow(q, id).Scan(&task.ID, &task.StatusID, &task.ProjectID, &task.PriorityID, &task.Name, &task.Description)
	return task, err
...
func (r *TasksRepo) Upd(t Task) error {
	...
	res, err := r.Exec(q, t.Name, t.Description, t.StatusID, t.PriorityID, t.ID)
	return verifyModified(res, err)
}

func NewTasksRepo(tx Tx) *TasksRepo {
	return &TasksRepo{initCtx(tx)}
}
